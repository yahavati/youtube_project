...
function LeftMenu() {
    ...
    const menuItems = [
        ...
        { type: "separator" },
        { text: "You", icon: "bi bi-chevron-right", className: "d-flex align-items-center" },
        { icon: "bi bi-images", text: "Your channel" },
        { icon: "bi bi-hourglass-split", text: "History" },
        { icon: "bi bi-music-note-list", text: "Playlists" },
        { icon: "bi bi-person-video2", text: "Your videos", onClick: navigateToYourVideos },
    ...
    return (
        <nav className="col-3 col-lg-12 bg-light vh-100 d-flex flex-column">
            ...
            <button onClick={scrollUp} className="btn white btn-light">▲</button>
            <ul ref={menuRef} className="list-group list-group-flush flex-grow-1 overflow-auto">
                {menuItems.map((item, index) => {
                    ...
                    if (item.type === "header") {
                        return <div key={index} className="list-group-item text-left">{item.text}</div>;
                    }
                    return (
                        <li key={index} className={`list-group-item my-group-item text-left ${item.className || ''}`}
