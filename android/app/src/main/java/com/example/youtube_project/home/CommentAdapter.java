    ...
    public static class CommentViewHolder extends RecyclerView.ViewHolder {
        ...
        public void bind(Comment comment, OnCommentClickListener listener) {
            ...
            if (comment.getUserPhoto() != null && !comment.getUserPhoto().isEmpty()) {
                byte[] decodedString = Base64.decode(comment.getUserPhoto(), Base64.DEFAULT);
                Bitmap decodedByte = BitmapFactory.decodeByteArray(decodedString, 0, decodedString.length);
                userPhoto.setImageBitmap(getCircularBitmap(decodedByte));
            } else {
                userPhoto.setImageResource(R.drawable.ic_circlr1); // Default user photo
            }

            // Set up the rest of the listeners and logic
            ...
            // Show edit and delete buttons only if the current user is the comment's owner
