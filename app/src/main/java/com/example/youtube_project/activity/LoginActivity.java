    ...
    private void performLogin(String username, String password) {
        new Thread(() -> {
            try {
                ...
                Response response = ApiHandler.sendPost("/login", payload);
                if (response.isSuccessful()) {
                    try {
                        String responseBody = response.body().string();
                        JsonObject jsonResponse = JsonParser.parseReader(new StringReader(responseBody)).getAsJsonObject();
                        boolean success = jsonResponse.get("success").getAsBoolean();
                        ApiHandler.setJwtToken(jsonResponse.get("token").getAsString());
                        Log.d("logger_info", jsonResponse.get("token").getAsString());
                        ...
                        if (success) {
                            runOnUiThread(() -> {
                                ...
                                UserDetails userDetails = new Gson().fromJson(jsonResponse.getAsJsonObject("user"), UserDetails.class);
                                userManager.setCurrentUser(username, userDetails);

                                // Add this line to notify listeners about the user details change
                            ...
                            runOnUiThread(() -> showErrorMessage(errorMsg));
                        }
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
                } else {
                    runOnUiThread(() -> showErrorMessage("Error: Unable to connect to the server."));
                }
            } catch (IOException e) {
                e.printStackTrace();
                runOnUiThread(() -> showErrorMessage("Error: " + e.getMessage()));
            }
        }).start();
    }
